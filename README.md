...
- **Twitch and Kick in one app.** A toggle in the header switches the entire
  app - home feed, browse, search, sidebar, and the watch box - between the two
  platforms (Twitch is the default; Kick mode turns the accent green).
...
- **Live-DVR seeking.** Seek backward on a live stream: within a rolling buffer
  instantly, and - if the broadcaster has VODs enabled - transparently further
  back against the in-progress recording.
...
under `src-tauri/target/release/bundle/` (e.g. `nsis/Mosaic_1.0.0_x64-setup.exe`,
`dmg/Mosaic_1.0.0_aarch64.dmg`).
...
hides the "Log in with Kick" button - Kick chat stays read-only rather than
