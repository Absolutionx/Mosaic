#!/usr/bin/env bash
# build-unix.sh - bootstrap and build the installer on macOS or Linux.
# Run from the project root: chmod +x build-unix.sh && ./build-unix.sh
#
# Installs system deps (Linux apt / macOS Homebrew + Xcode CLT), Rust (rustup),
# and Node.js if missing, runs npm install + `npm run tauri build`, then prints
# the finished installer's path. End users who install the output need none of
# this.
